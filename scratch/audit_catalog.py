...
def is_valid_game_title(col_letter, val, dev_raw_name, dev_clean_name):
    if not val:
        return False
    # Only columns D, F, H contain game titles
    if col_letter not in ['D', 'F', 'H']:
        return False
    # Ignore checkmarks or symbols
    if val.startswith('✅') or val == '✅':
        return False
    # Ignore numbers like 4.0, 5.0, 9.00, 11.00, etc.
    if re.match(r'^\d+(\.\d+)?$', val):
        return False
    # Ignore headers or branch text
    if val in [dev_raw_name, dev_clean_name] or 'DSTERGAME' in val.upper() or 'HAK MEMBER' in val.upper() or 'WA admin' in val or 'Jl.' in val:
        return False
    if len(val) < 2:
        return False
    return True
...
for r_num in sorted(sheet_map.keys()):
    ...
    if current_device_obj:
        for col_let, cell_val in row.items():
            if is_valid_game_title(col_let, cell_val, current_device_obj["rawName"], current_device_obj["name"]):
                c_title = clean_game_title(cell_val)
                if c_title and c_title not in current_device_obj["games"]:
                    current_device_obj["games"].append(c_title)

# Filter empty devices
...
with open('scratch_audit_results.txt', 'w', encoding='utf-8') as out:
    out.write(f"TOTAL DEVICES: {len(devices_data)}\n\n")
    for d in devices_data:
        out.write(f"Device: '{d['name']}' (raw: '{d['rawName']}') -> {len(d['games'])} games\n")
        out.write(f"   Games: {d['games']}\n\n")

with open('src/data/game-catalog.json', 'w', encoding='utf-8') as out_f:
    json.dump(devices_data, out_f, ensure_ascii=False, indent=2)

print("Audit complete! Results written to scratch_audit_results.txt and updated src/data/game-catalog.json")
