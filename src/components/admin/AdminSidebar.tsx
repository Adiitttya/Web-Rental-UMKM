...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ isMobileOpen = false, onClose }) => {
  ...
  return (
    <>
      ...
      {isMobileOpen && (
        <div
          onClick={onClose}
          className="fixed inset-0 bg-slate-900/80 z-30 lg:hidden transition-opacity"
        />
      )}

      <aside
        className={`w-64 bg-slate-900 text-slate-100 flex flex-col shrink-0 h-full overflow-hidden shadow-xl border-r border-slate-800 z-40 transition-transform duration-300 ease-in-out fixed inset-y-0 left-0 lg:static lg:translate-x-0 ${
      ...
      >
        ...
        <div className="p-5 border-b border-slate-800 flex items-center justify-between shrink-0">
          <Link href="/admin/dashboard" onClick={onClose} className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-[var(--primary)] flex items-center justify-center font-black text-white text-base shadow-lg shadow-[var(--primary)]/30">
              D
            </div>
            <div>
              <h1 className="font-extrabold text-sm leading-none text-white">DsterGame Studio</h1>
              <span className="text-[10px] text-blue-400 font-bold tracking-wider uppercase">CMS Panel</span>
            </div>
          </Link>
        ...
        <nav className="flex-1 p-3.5 space-y-4 overflow-y-auto admin-sidebar-scrollbar">
          {navSections.map((section) => (
            <div key={section.title} className="space-y-1">
              <div className="px-3 pt-1 pb-1 text-[10px] font-extrabold text-slate-500 tracking-wider uppercase">
                {section.title}
              </div>
        ...
        <div className="p-3 border-t border-slate-800 shrink-0">
          <Link
            href="/"
            target="_blank"
            className="flex items-center justify-center gap-2 w-full py-2 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs transition-colors border border-slate-700/50"
          >
